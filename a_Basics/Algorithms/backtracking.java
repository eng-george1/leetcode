...
public class backtracking {
    ...
    public static void main(String[] args) {
        ...
        generatePermutationsAllOrder(arr, new ArrayList<Integer>(), result,0);
     ...
     * #Idea:
     */
    // Recursive function to generate all permutations with the full size
    private static void generatePermutations(int[] arr, List<Integer> current, List<List<Integer>> result) {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            current.remove(current.size() - 1); // Backtrack for the mail thread it always have one item
        }
    }
      // Recursive function to generate all permutations with any size
      private static void generatePermutationsAll(int[] arr, List<Integer> current, List<List<Integer>> result) {
        // Base case: if the current permutation has the same length as the array, add
        // it to result 
        if(current.size()>0) 
        result.add(new ArrayList<>(current));
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            // Choose the element and add it to the current permutation
            current.add(arr[i]);
            generatePermutationsAll(arr, current, result); // Recurse
            current.remove(current.size() - 1); // Backtrack for the mail thread it always have one item
        }
    }
       // Recursive function to generate all permutations with any size but order is not important
       private static void generatePermutationsAllOrder(int[] arr, List<Integer> current, List<List<Integer>> result, int startIndex) {
        // Base case: if the current permutation has the same length as the array, add
        // it to result 
        if(current.size()>0) 
        result.add(new ArrayList<>(current));
        ...
        for (int i = startIndex; i < arr.length; i++) {
            ...
            // Choose the element and add it to the current permutation
            current.add(arr[i]);
            generatePermutationsAllOrder(arr, current, result,i+1); // Recurse
            current.remove(current.size() - 1); // Backtrack for the mail thread it always have one item
        }
    }
